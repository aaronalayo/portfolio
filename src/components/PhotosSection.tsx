...
const PhotosSection = () => {
  ...
  return (
    <>
      {/* --- SITE_URL IMPLEMENTATION START --- */}
      {selectedPhoto ? (
        <>
          <title>{`${selectedPhoto.title} - Photo by Aaron Alayo`}</title>
          <meta name="description" content={`View the photograph "${selectedPhoto.title}" in the photography gallery.`} />
          <link rel="canonical" href={`${SITE_URL}/photos/${selectedPhoto.slug.current}`} />
        </>
      ) : (
        <>
          <title>Photography - Aaron Alayo</title>
          <meta name="description" content="Professional photography portfolio of Aaron Alayo." />
          <link rel="canonical" href={`${SITE_URL}/photos`} />
        </>
      )}
      {/* --- SITE_URL IMPLEMENTATION END --- */}
          ...
          {categoryOptions.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-full border text-xs uppercase tracking-widest transition-all ${
                selectedCategory === cat ? 'bg-black text-white border-black' : 'bg-white text-black border-black/20 hover:border-black'
              }`}
            >
              {cat}
            </button>
        ...
        {selectedPhoto && (
          ...
          >
            ...
            <div className="w-full max-w-5xl h-24 flex items-center justify-between px-4 shrink-0 pointer-events-auto">
              <button onClick={(e) => { e.stopPropagation(); handlePrev(); }} className="text-4xl sm:text-5xl text-black hover:scale-125 transition-transform cursor-pointer p-4">‹</button>
              <div className="flex flex-col items-center">
                 <p className="font-veep uppercase tracking-[0.2em] text-[10px] sm:text-xs text-black text-center px-4">
                  {selectedPhoto.title}
                </p>
              </div>
              <button onClick={(e) => { e.stopPropagation(); handleNext(); }} className="text-4xl sm:text-5xl text-black hover:scale-125 transition-transform cursor-pointer p-4">›</button>
            </div>
          </div>
        )}
      </section>
